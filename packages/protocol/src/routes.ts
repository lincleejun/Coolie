...
export const ROUTES = [
  ...
  },
  {
    ...
    description: "创建 PR 和/或合回主 checkout；不删除 worktree，结果写入 Workspace.finishResult",
    request: "path: id + finish options",
    response: "FinishResult",
    sideEffects: "git/PR operations; persists finishResult; does not archive",
  },
  {
    method: "DELETE",
    path: "/workspaces/:id/finish-result",
    name: "delete.workspaces.finish-result",
    description: "Dismiss finish success panel (Keep working) without archiving",
    request: "path: id",
    response: "Workspace",
    sideEffects: "clears data.finishResult only",
  },
  {
    method: "POST",
