...
const out = resolve(process.argv[2] ?? join(root, "packages/server/dist/sidecar-spike"))
...
const copyNativeClosure = (destination) => {
  ...
  const sqlite = packageRoot("better-sqlite3")
...
copy(process.execPath, join(runtime, "node"))
...
copy(process.execPath, executable)
